...
class BankAccount:
    ...
    def __del__(self):
        print(f"{self.name} {self.surname} account has been destroyed")
...
while True:
    ...
    if command==2:
        if len(bank_accounts)==0:
            ...
            account_num=int(input("Напишите номер пользователя:"))

            account=bank_accounts[account_num-1]

            while True:
                action=int(input(
                    "Выберите ваше действие:\n1. Пополнить баланс\n2. Снять деньги из баланса\n"
                    "3. Конвертировать баланс\n4. Вывести баланс\n5. Установить баланс\n6. Закончить сессию\n"
                ))
